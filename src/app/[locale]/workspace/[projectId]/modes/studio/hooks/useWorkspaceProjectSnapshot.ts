...
export function useWorkspaceProjectSnapshot({
  ...
  episode,
  urlStage,
}: Pick<StudioWorkspaceProps, 'project' | 'episode' | 'urlStage'>) {
  return useMemo(() => {
    const projectData = project.studioData
  ...
  }, [episode?.novelText, episode?.storyboards, project.studioData, urlStage])
}
