...
import { StudioStoryboard } from '@/types/project'
...
function buildStoryboardTextTargets(storyboards: StudioStoryboard[]): TaskTarget[] {
  const targets: TaskTarget[] = []

  for (const storyboard of storyboards) {
    targets.push({
      key: `storyboard:${storyboard.id}`,
      targetType: 'StudioStoryboard',
...
function buildPanelTargets(storyboards: StudioStoryboard[], type: 'image' | 'video' | 'lip-sync'): TaskTarget[] {
  const targets: TaskTarget[] = []
