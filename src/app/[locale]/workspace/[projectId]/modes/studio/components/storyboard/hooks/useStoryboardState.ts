...
import { StudioStoryboard, StudioClip, StudioPanel } from '@/types/project'
...
interface UseStoryboardStateProps {
  projectId: string
  episodeId: string
  initialStoryboards: StudioStoryboard[]
  clips: StudioClip[]
}
...
export function useStoryboardState({
  ...
  const setLocalStoryboards = useCallback<React.Dispatch<React.SetStateAction<StudioStoryboard[]>>>(
    (nextStoryboardsOrUpdater) => {
      const resolveNextStoryboards = (previousStoryboards: StudioStoryboard[]) => (
        typeof nextStoryboardsOrUpdater === 'function'
          ? (nextStoryboardsOrUpdater as (previous: StudioStoryboard[]) => StudioStoryboard[])(previousStoryboards)
          : nextStoryboardsOrUpdater
      )

      queryClient.setQueryData(queryKeys.episodeData(projectId, episodeId), (previous: unknown) => {
        if (!previous || typeof previous !== 'object') return previous
        const episode = previous as { storyboards?: StudioStoryboard[] }
      ...
      queryClient.setQueryData(queryKeys.storyboards.all(episodeId), (previous: unknown) => {
        if (!previous || typeof previous !== 'object') return previous
        const payload = previous as { storyboards?: StudioStoryboard[] }
  ...
  const getPanelImages = (storyboard: StudioStoryboard): Array<string | null> => {
  ...
  const getTextPanels = (storyboard: StudioStoryboard): StoryboardPanel[] => {
    const panels = getStoryboardPanels(storyboard)
    const sortedPanels = [...panels].sort((a: StudioPanel, b: StudioPanel) =>
