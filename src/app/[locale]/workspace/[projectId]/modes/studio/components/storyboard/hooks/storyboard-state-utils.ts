...
import type { StudioClip, StudioPanel, StudioStoryboard } from '@/types/project'
...
export function computeTotalPanels(storyboards: StudioStoryboard[]): number {
  return storyboards.reduce((sum, storyboard) => sum + (getStoryboardPanels(storyboard).length || storyboard.panelCount || 0), 0)
}

export function formatClipTitle(clip: StudioClip | null | undefined): string {
  if (!clip) return '-'
