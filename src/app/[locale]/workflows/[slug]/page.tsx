...
export default function WorkflowDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Compact header */}
      <div className="px-6 lg:px-10 pt-3 pb-4 max-w-[1280px] mx-auto w-full">
        {/* Breadcrumb + tags row */}
        ...
        <p className="text-[14px] text-[#666] leading-relaxed max-w-[800px]">{workflow.description}</p>
      </div>

      {/* Main body: left tabs + right run panel */}
      <div className="flex-1 px-6 lg:px-10 max-w-[1280px] mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 h-full">

          {/* Left column: Why + Tabs */}
            ...
            <div className="flex gap-0 border-b border-[#e5e5e5] mb-3">
              {TABS.map((tab) => (
                <button
                  ...
                  className={`px-4 py-2 text-[13px] font-medium border-b-2 transition -mb-px ${
                    activeTab === tab
                ...
                <div className="flex flex-col gap-2">
                  {workflow.prompts.map((p, i) => (
                    <div key={i} className="bg-[#fafafa] border border-[#eee] rounded-xl p-3">
                      ...
                      <pre className="bg-white border border-[#eee] rounded-md p-2.5 text-[12px] text-[#333] font-mono whitespace-pre-wrap leading-relaxed max-h-[200px] overflow-y-auto">
                        {p.text}
                      </pre>
                    </div>
                  ))}
              ...
              {activeTab === 'tips' && (
                <div className="flex flex-col gap-2">
                  {workflow.tips.map((tip, i) => (
                    <div
                      key={i}
                      className={`p-3 border rounded-lg text-[13px] leading-relaxed ${
                        i === 0
                          ? 'border-l-2 border-l-purple-500 border-[#e5e5e5] bg-[#faf5ff] text-[#3b0764]'
          ...
          <aside className="lg:sticky lg:top-16 self-start bg-white border border-[#e5e5e5] rounded-2xl p-4">
                ...
                <div className="flex gap-1">
                  {RATIO_OPTIONS.map((r) => (
                    <button
                      key={r}
                      onClick={() => setRatio(r)}
                      className={`flex-1 py-1.5 rounded-lg text-[11px] border text-center transition ${
            ...
            {wf.status !== 'idle' && (
              <button
                onClick={wf.reset}
                className="w-full mt-1.5 py-1.5 text-[12px] text-[#555] hover:text-[#0a0a0a] transition"
              >
                {t('reset')}
              </button>
            )}

            <div className="mt-3 pt-3 border-t border-[#f0f0f0] text-[11px] text-[#777] leading-relaxed">
      ...
      {/* Result area */}
      {wf.status !== 'idle' && (
        <div ref={resultRef} className="px-6 lg:px-10 py-6 max-w-[1280px] mx-auto w-full">
          <h3 className="font-mono text-base font-normal tracking-tight mb-3">{t('result')}</h3>
          ...
          {(wf.status === 'generating_music' || wf.status === 'music_done' || wf.musicUrl) && (
            <div className="mb-3 p-3 bg-gradient-to-r from-[#faf5ff] to-[#f3e8ff] border border-[#e9d5ff] rounded-xl flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-3 items-center">
            <div
              className={`bg-[#fafafa] border border-[#e5e5e5] rounded-2xl aspect-video overflow-hidden flex items-center justify-center ${wf.imageUrl ? 'cursor-zoom-in group' : ''}`}
              onClick={() => wf.imageUrl && setZoomMedia({ type: 'image', url: wf.imageUrl })}
            >
              {wf.imageUrl ? (
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition flex items-center justify-center">
                    <AppIcon name="search" className="w-8 h-8 text-white opacity-0 group-hover:opacity-80 transition drop-shadow-lg" />
                  </div>
                </div>
              ) : (
            ...
            <div
              className={`bg-gradient-to-br from-[#1a0f2a] to-[#0a0515] border border-[#e5e5e5] rounded-2xl aspect-video overflow-hidden flex items-center justify-center relative ${wf.videoUrl ? 'cursor-zoom-in group' : ''}`}
              onClick={() => wf.videoUrl && setZoomMedia({ type: 'video', url: wf.videoUrl })}
            >
              {wf.videoUrl ? (
                <div className="relative w-full h-full">
                  <video src={wf.videoUrl} controls autoPlay className="w-full h-full object-cover" />
                  <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-80 transition">
          ...
          {wf.error && (
            wf.error === 'API_KEY_REQUIRED' ? (
              <div className="mt-3 p-3 bg-amber-50 border border-amber-200 text-amber-800 text-[13px] rounded-lg flex items-center justify-between">
              ...
              <div className="mt-3 p-2.5 bg-red-50 border border-red-200 text-red-600 text-[13px] rounded-lg">
                {wf.error}
              </div>
            )
          )}
          ...
          {wf.videoUrl && (
            <div className="mt-3 flex gap-3">
              <a href={wf.videoUrl} download target="_blank" rel="noopener noreferrer"
                className="px-5 py-2 bg-[#0a0a0a] text-white rounded-full text-[13px] font-medium hover:bg-[#333] transition">
                {t('downloadVideo')}
              </a>
              {wf.imageUrl && (
                <a href={wf.imageUrl} download target="_blank" rel="noopener noreferrer"
                  className="px-5 py-2 bg-white text-[#0a0a0a] border border-[#d4d4d4] rounded-full text-[13px] font-medium hover:border-[#0a0a0a] transition">
                  {t('downloadImage')}
                </a>
              )}
            </div>
      ...
      {/* Related workflows */}
      <div className="mt-auto py-6 px-6 lg:px-10 bg-[#fafafa] border-t border-[#f0f0f0]">
      ...
      {zoomMedia && (
        ...
        >
          ...
          <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-3">
            <a
              href={zoomMedia.url} download target="_blank" rel="noopener noreferrer"
