...
/**
 * GET /api/studio/[projectId]/voice-lines?episodeId=xxx
 * 获取剧集的台词列表
 */
export const GET = apiHandler(async (
  ...
  if (speakersOnly === '1') {
    const novelProject = await prisma.studioProject.findUnique({
      where: { projectId },
      select: { id: true }
    })
    if (!novelProject) {
    ...
    const speakerRows = await prisma.studioVoiceLine.findMany({
      where: {
        episode: {
          studioProjectId: novelProject.id
        }
      },
      select: { speaker: true },
      distinct: ['speaker'],
  ...
  const voiceLines = await prisma.studioVoiceLine.findMany({
    where: { episodeId },
...
/**
 * POST /api/studio/[projectId]/voice-lines
 * 新增单条台词
...
export const POST = apiHandler(async (
  ...
  const maxLine = await prisma.studioVoiceLine.findFirst({
    where: { episodeId },
  ...
  const created = await prisma.studioVoiceLine.create({
    data: {
      episodeId,
...
/**
 * PATCH /api/studio/[projectId]/voice-lines
...
export const PATCH = apiHandler(async (
  ...
  // 单条更新
  if (lineId) {
    const updateData: Prisma.StudioVoiceLineUncheckedUpdateInput = {}
    ...
    if (matchedPanelId !== undefined) {
      const currentLine = await prisma.studioVoiceLine.findUnique({
        where: { id: lineId },
    ...
    const updated = await prisma.studioVoiceLine.update({
      where: { id: lineId },
      data: updateData,
      include: {
        matchedPanel: {
  ...
  if (speaker && episodeId) {
    const result = await prisma.studioVoiceLine.updateMany({
      where: {
        episodeId,
        speaker
      },
...
/**
 * DELETE /api/studio/[projectId]/voice-lines?lineId=xxx
 * 删除单条台词
 */
export const DELETE = apiHandler(async (
  ...
  // 获取要删除的台词
  const lineToDelete = await prisma.studioVoiceLine.findUnique({
    where: { id: lineId }
  })
  ...
  // 删除台词
  await prisma.studioVoiceLine.delete({
    where: { id: lineId }
  })

  // 重新排序剩余台词的 lineIndex
  const remainingLines = await prisma.studioVoiceLine.findMany({
