...
export const POST = apiHandler(async (
  ...
  if (!storyboard || storyboard.episode.studioProject.projectId !== projectId) {
  ...
  const insertAfter = await prisma.studioPanel.findUnique({ where: { id: insertAfterPanelId } })
  ...
  const createdPanel = await prisma.$transaction(async (tx) => {
    ...
    const created = await tx.studioPanel.create({
      data: {
        id: createdPanelId,
        storyboardId,
