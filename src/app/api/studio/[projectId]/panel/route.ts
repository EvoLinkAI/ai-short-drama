...
import { serializeStructuredJsonField } from '@/lib/studio/panel-ai-data-sync'
...
/**
 * POST /api/studio/[projectId]/panel
 * 新增一个 Panel
 */
export const POST = apiHandler(async (
  ...
  const panelModel = prisma.studioPanel as unknown as {
  ...
  // 更新 panelCount
  const panelCount = await prisma.studioPanel.count({
    where: { storyboardId }
  })
...
/**
 * DELETE /api/studio/[projectId]/panel
 * 删除一个 Panel
 */
export const DELETE = apiHandler(async (
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const maxPanel = await tx.studioPanel.findFirst({
      where: { storyboardId },
      orderBy: { panelIndex: 'desc' },
    ...
    // 阶段1：整体上移，避免与原索引冲突
    await tx.studioPanel.updateMany({
      where: {
        storyboardId,
    ...
    // 3. 获取更新后的 panel 总数
    const panelCount = await tx.studioPanel.count({
      where: { storyboardId }
    })
...
/**
 * PATCH /api/studio/[projectId]/panel
...
export const PATCH = apiHandler(async (
  ...
  // 尝试更新 Panel
  const updatedPanel = await prisma.studioPanel.updateMany({
    where: {
      storyboardId,
      panelIndex
    },
...
/**
 * PUT /api/studio/[projectId]/panel
...
export const PUT = apiHandler(async (
  ...
  // 查找现有 Panel
  const existingPanel = await prisma.studioPanel.findUnique({
    where: {
