...
export const PATCH = apiHandler(async (
  ...
  const updateData: Prisma.StudioEpisodeUncheckedUpdateInput = {}
  ...
  const episode = await prisma.studioEpisode.update({
    where: { id: episodeId },
    data: updateData
  })
...
export const DELETE = apiHandler(async (
  ...
  // 删除剧集（关联数据会级联删除）
  await prisma.studioEpisode.delete({
    where: { id: episodeId }
  })
  ...
  if (studioProject?.lastEpisodeId === episodeId) {
    // 找到另一个剧集作为默认
    const anotherEpisode = await prisma.studioEpisode.findFirst({
      where: { studioProjectId: studioProject.id },
