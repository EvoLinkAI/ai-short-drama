...
import { StudioStoryboard, StudioPanel } from './project'

/**
 * 带有已加载 panels 的 Storyboard 类型
 * 用于数据库查询后包含 panels 的情况
 */
export interface StoryboardWithPanels extends StudioStoryboard {
    panels: StudioPanel[]
}
 ...
 * 如果 panels 不存在则返回空数组
 */
export function getPanels(storyboard: StudioStoryboard): StudioPanel[] {
 ...
 */
export function getPanelCandidates(panel: StudioPanel): string[] {
