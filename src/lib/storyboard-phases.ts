...
export async function executePhase1(
    ...
    logAIAnalysis(session.user.id, session.user.name, projectId, projectName, {
        ...
        model: studioData.analysisModel
    })
...
export async function executePhase2(
    ...
    const filteredFullDescription = getFilteredFullDescription(studioData.characters, clipCharacters)
    const filteredLocationsDescription = getFilteredLocationsDescription(studioData.locations, clipLocation)
    const filteredPropsDescription = compileAssetPromptFragments(buildPromptAssetContext({
        characters: [],
        locations: [],
        props: studioData.props || [],
