    ...
    return useMutation({
        mutationFn: async (payload: {
            ...
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/ai-modify-shot-prompt`,
                {
                    method: 'POST',
...
export function useAnalyzeProjectShotVariants(projectId: string) {
    return useMutation({
        mutationFn: async (payload: { panelId: string }) => {
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/analyze-shot-variants`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
    ...
    return useMutation({
        mutationFn: async (payload: {
            ...
            await requestJsonWithError(
                `/api/studio/${projectId}/photography-plan`,
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
...
export function useSelectProjectPanelCandidate(projectId: string) {
    ...
    return useMutation({
        mutationFn: async (payload: {
            ...
            await requestJsonWithError(
                `/api/studio/${projectId}/panel/select-candidate`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
