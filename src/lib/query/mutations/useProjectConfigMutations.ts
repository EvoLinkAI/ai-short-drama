    ...
    return useMutation({
        mutationFn: async () => {
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/analyze-global`,
                {
                    method: 'POST',
    ...
    return useMutation({
        mutationFn: async ({ episodeId }: { episodeId: string }) => {
            const data = await requestJsonWithError<{ storyboards?: Array<{ panels?: unknown[] }> }>(
                `/api/studio/${projectId}/storyboards?episodeId=${encodeURIComponent(episodeId)}`,
                { method: 'GET' },
