...
function applyCharacterSelectionToProject(
    ...
    const currentCharacters = previous.studioData.characters || []
    return {
        ...previous,
        studioData: {
            ...previous.studioData,
...
export function useUploadProjectCharacterImage(projectId: string) {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            return await requestJsonWithError(`/api/studio/${projectId}/upload-asset-image`, {
                method: 'POST',
                body: formData
    ...
    return useMutation({
        mutationFn: async (characterId: string) => {
            await requestVoidWithError(
                `/api/studio/${projectId}/character?id=${encodeURIComponent(characterId)}`,
                { method: 'DELETE' },
...
export function useDeleteProjectAppearance(projectId: string) {
    ...
    return useMutation({
        mutationFn: async ({ characterId, appearanceId }: { characterId: string; appearanceId: string }) => {
            await requestVoidWithError(
                `/api/studio/${projectId}/character/appearance?characterId=${encodeURIComponent(characterId)}&appearanceId=${encodeURIComponent(appearanceId)}`,
                { method: 'DELETE' },
