...
export async function runScriptToStoryboardAtomicRetry(params: {
  ...
  const filteredFullDescription = getFilteredFullDescription(params.studioData.characters || [], clipCharacters)
  const filteredLocationsDescription = getFilteredLocationsDescription(
    params.studioData.locations || [],
    clipLocation,
  )
  const filteredPropsDescription = compileAssetPromptFragments(buildPromptAssetContext({
    characters: [],
    locations: [],
    props: params.studioData.props || [],
    clipCharacters: [],
    clipLocation: null,
    clipProps,
  ...
  if (params.retryTarget.phase === 'phase1') {
    ...
    const filteredAppearanceList = getFilteredAppearanceList(params.studioData.characters || [], clipCharacters)
    const charactersLibName = (params.studioData.characters || []).map((item) => item.name).join(', ') || '无'
    const locationsLibName = (params.studioData.locations || []).map((item) => item.name).join(', ') || '无'
    const charactersIntroduction = buildCharactersIntroduction(params.studioData.characters || [])
