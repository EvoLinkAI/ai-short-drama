...
async function diagnoseProject(projectId: string) {
  ...
  // 2. 检查 StudioProject
  console.log('\n2️⃣ Studio 项目配置:')
  const novelData = project.studioData
  if (!novelData) {
    console.log('  ❌ studioData 未创建')
  } else {
  ...
  if (!novelProjectId) {
    console.log('  ❌ 无法获取 studioProject ID')
    process.exit(1)
  }
  
  const locations = await prisma.studioLocation.findMany({
    where: { studioProjectId: novelProjectId },
    include: {
      images: {
