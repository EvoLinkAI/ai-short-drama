...
type MigrationIssue = {
  table: 'userPreference' | 'studioProject'
  rowId: string
  userId?: string
  field: string
  kind:
...
async function main() {
  ...
  for (const row of projects as ProjectRow[]) {
    report.studioProject.scanned += 1
    ...
    for (const field of MODEL_FIELDS) {
      ...
      if (normalizedField.changed) {
        updateData[field] = normalizedField.nextValue
        report.studioProject.updatedFields += 1
      }
    }

    if (Object.keys(updateData).length > 0) {
      report.studioProject.updated += 1
      if (APPLY) {
  ...
  process.stdout.write(
    ...
    `projects=${report.studioProject.scanned}/${report.studioProject.updated} ` +
