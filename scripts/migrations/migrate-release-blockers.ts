...
async function migrateNovelProjects(summary: MigrationSummary): Promise<void> {
  ...
  summary.studioProject.scanned = rows.length
  ...
  for (const row of rows) {
    ...
    for (const field of PROJECT_MODEL_FIELDS) {
      ...
      changed = true
      if (fieldResult.migrated) {
        summary.studioProject.migratedModelFields += 1
      }
      if (fieldResult.clearedInvalid) {
        summary.studioProject.invalidModelFieldsCleared += 1
      }
    }
    ...
    if (!capabilityResult.ok) {
      ...
      summary.studioProject.dirtyClearedCapabilityOverrides += 1
    } else if (capabilityResult.changed) {
      updateData.capabilityOverrides = capabilityResult.nextRaw
      changed = true
      summary.studioProject.migratedCapabilityOverrideKeys += capabilityResult.migratedKeys
    }

    if (!changed) continue
    summary.studioProject.updated += 1
