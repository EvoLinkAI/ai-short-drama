...
const lineScanRoots = [
  'src/app/[locale]/workspace/[projectId]/modes/studio',
  'src/lib/query/hooks',
]
...
function collectFileViolations(fullPath) {
  ...
  const updateCallRegex = /studioProject\.update\(\{[\s\S]*?\n\s*\}\)/g
  for (const match of content.matchAll(updateCallRegex)) {
    ...
    violations.push(`${relPath}:${lineNumber} forbidden: DB stage write in studioProject.update`)
  }
