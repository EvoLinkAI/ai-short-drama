...
export async function createFixtureProject(userId: string, mode: 'studio' | 'general' = 'studio') {
  const id = suffix()
...
export async function createFixtureEpisode(studioProjectId: string, episodeNumber = 1) {
  return await prisma.studioEpisode.create({
    data: {
      studioProjectId,
      episodeNumber,
