...
describe('api specific - unified assets routes', () => {
  ...
  it('POST /api/studio/[projectId]/copy-from-global delegates to the centralized copy service', async () => {
    const mod = await import('@/app/api/studio/[projectId]/copy-from-global/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/copy-from-global',
      method: 'POST',
      body: {
        type: 'voice',
