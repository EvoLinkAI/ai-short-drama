...
describe('api specific - studio location style forwarding', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('does not auto-generate images when creating location', async () => {
    ...
    const mod = await import('@/app/api/studio/[projectId]/location/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/location',
      method: 'POST',
      headers: {
  ...
  it('rejects invalid artStyle before creating location', async () => {
    ...
    expect(prismaMock.studioLocation.create).not.toHaveBeenCalled()
    expect(fetchMock).not.toHaveBeenCalled()
  })
