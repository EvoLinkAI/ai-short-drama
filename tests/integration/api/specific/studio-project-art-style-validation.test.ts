...
const authMock = vi.hoisted(() => ({
  requireProjectAuthLight: vi.fn(async () => ({
    ...
    project: { id: 'project-1', userId: 'user-1', mode: 'studio', name: 'Project 1' },
  })),
...
describe('api specific - studio project art style validation', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('accepts valid artStyle and keeps user preference unchanged', async () => {
    const mod = await import('@/app/api/studio/[projectId]/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1',
      method: 'PATCH',
      body: {
        artStyle: '  realistic  ',
      },
  ...
  it('rejects invalid artStyle with invalid params', async () => {
    ...
    expect(prismaMock.studioProject.update).not.toHaveBeenCalled()
    expect(prismaMock.userPreference.upsert).not.toHaveBeenCalled()
  })
