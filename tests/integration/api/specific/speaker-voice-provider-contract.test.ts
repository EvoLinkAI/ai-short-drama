...
describe('api specific - speaker voice provider contract', () => {
  ...
  it('returns INVALID_PARAMS when provider is missing in PATCH payload', async () => {
    const mod = await import('@/app/api/studio/[projectId]/speaker-voice/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/speaker-voice',
      method: 'PATCH',
      body: {
        episodeId: 'episode-1',
        speaker: 'Narrator',
    ...
    expect(prismaMock.studioEpisode.update).not.toHaveBeenCalled()
  })
  ...
  it('stores fal speaker voice with explicit provider and normalized audio storage key', async () => {
    ...
    const updateCall = prismaMock.studioEpisode.update.mock.calls[0] as
