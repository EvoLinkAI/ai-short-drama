...
describe('api specific - studio generate image art style', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('accepts valid artStyle and forwards it into task payload', async () => {
    const mod = await import('@/app/api/studio/[projectId]/generate-image/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/generate-image',
      method: 'POST',
      body: {
        type: 'character',
        id: 'character-1',
