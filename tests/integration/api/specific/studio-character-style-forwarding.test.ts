...
describe('api specific - studio character style forwarding', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('does not auto-generate images when creating by text prompt', async () => {
    ...
    const mod = await import('@/app/api/studio/[projectId]/character/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/character',
      method: 'POST',
      headers: {
  ...
  it('rejects invalid artStyle before creating character', async () => {
    ...
    expect(prismaMock.studioCharacter.create).not.toHaveBeenCalled()
    expect(fetchMock).not.toHaveBeenCalled()
  })
})
