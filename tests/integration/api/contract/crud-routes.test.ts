  ...
  return {
    ...
    requireProjectAuth: async (projectId: string) => {
      ...
      return {
        ...
        project: { id: projectId, userId: 'user-1', mode: 'studio' },
      }
    },
...
describe('api contract - crud routes (behavior)', () => {
  const routes = ROUTE_CATALOG.filter(
    (entry) => (
      ...
      || entry.contractGroup === 'crud-studio-routes'
    ),
  )
  ...
  beforeEach(() => {
    ...
    prismaMock.studioLocation.update.mockResolvedValue({
      id: 'location-1',
      selectedImageId: 'img-1',
    })
    prismaMock.studioClip.update.mockResolvedValue({
      id: 'clip-1',
    ...
    prismaMock.studioStoryboard.findUnique.mockResolvedValue({
      id: 'storyboard-1',
      projectId: 'project-1',
    })
    prismaMock.studioStoryboard.update.mockResolvedValue({
      id: 'storyboard-1',
      panelCount: 1,
    })
    ...
    prismaMock.studioPanel.update.mockResolvedValue({
      id: 'panel-1',
      storyboardId: 'storyboard-1',
      panelIndex: 0,
      props: JSON.stringify(['Bronze Dagger']),
    })
    prismaMock.studioPanel.create.mockResolvedValue({
      id: 'panel-2',
      storyboardId: 'storyboard-1',
      panelIndex: 1,
      props: JSON.stringify(['Bronze Dagger']),
    })
    prismaMock.studioPanel.count.mockResolvedValue(1)
  })
  ...
  it('POST /studio/[projectId]/select-character-image writes selectedIndex and imageUrl key', async () => {
    authState.authenticated = true
    const mod = await import('@/app/api/studio/[projectId]/select-character-image/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/select-character-image',
      method: 'POST',
      body: {
        characterId: 'character-1',
        appearanceId: 'appearance-1',
  ...
  it('POST /studio/[projectId]/select-location-image toggles selected state and selectedImageId', async () => {
    authState.authenticated = true
    const mod = await import('@/app/api/studio/[projectId]/select-location-image/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/select-location-image',
      method: 'POST',
      body: {
        locationId: 'location-1',
        selectedIndex: 1,
    ...
    expect(prismaMock.studioLocation.update).toHaveBeenCalledWith({
      where: { id: 'location-1' },
      data: { selectedImageId: 'img-1' },
    })
  })

  it('PATCH /studio/[projectId]/clips/[clipId] writes provided editable fields', async () => {
    authState.authenticated = true
    const mod = await import('@/app/api/studio/[projectId]/clips/[clipId]/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/clips/clip-1',
      method: 'PATCH',
      body: {
        characters: JSON.stringify(['Alice']),
        location: 'Old Town',
    ...
    expect(res.status).toBe(200)
    expect(prismaMock.studioClip.update).toHaveBeenCalledWith({
      where: { id: 'clip-1' },
      data: {
        characters: JSON.stringify(['Alice']),
        location: 'Old Town',
  ...
  it('PUT /studio/[projectId]/panel writes provided props to prisma.studioPanel.update', async () => {
    authState.authenticated = true
    const mod = await import('@/app/api/studio/[projectId]/panel/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/panel',
      method: 'PUT',
      body: {
        storyboardId: 'storyboard-1',
        panelIndex: 0,
