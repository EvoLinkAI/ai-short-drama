...
describe('worker character-profile behavior', () => {
  beforeEach(() => {
    ...
    prismaMock.studioCharacter.findFirst.mockImplementation(async (args: { where: { id: string } }) => ({
      id: args.where.id,
    ...
    prismaMock.studioCharacter.findMany.mockResolvedValue([
      {
        id: 'character-1',
        name: 'Hero',
        profileData: JSON.stringify({ archetype: 'lead' }),
  ...
  it('confirm profile success -> rebuilds appearances and marks profileConfirmed', async () => {
    ...
    expect(prismaMock.studioCharacter.update).toHaveBeenCalledWith({
