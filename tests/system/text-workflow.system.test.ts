...
vi.mock('@/lib/studio/script-to-storyboard/orchestrator', async () => {
  const actual = await vi.importActual<typeof import('@/lib/studio/script-to-storyboard/orchestrator')>(
    '@/lib/studio/script-to-storyboard/orchestrator',
  )
  return {
    ...actual,
...
vi.mock('@/lib/workers/handlers/script-to-storyboard-helpers', async () => {
  ...
  return {
    ...
    persistStoryboardsAndPanels: vi.fn(async (input: { episodeId: string }) => {
      const clip = await prisma.studioClip.findFirst({
      ...
      const panel = await prisma.studioPanel.create({
        data: {
          id: 'panel-1',
...
describe('system - text workflows', () => {
  ...
  it('script-to-storyboard success -> persists storyboard/panel/voiceLine and completes task', async () => {
    ...
    const mod = await import('@/app/api/studio/[projectId]/script-to-storyboard-stream/route')
    ...
    const persistedVoiceLines = await prisma.studioVoiceLine.findMany({
      where: { episodeId: seeded.episode.id },
  ...
  it('insert-panel invalid ai payload -> task fails and no dirty panel remains', async () => {
    ...
    const beforeCount = await prisma.studioPanel.count({
      where: { storyboardId: seeded.storyboard.id },
    })

    const mod = await import('@/app/api/studio/[projectId]/insert-panel/route')
    const response = await callRoute(
      mod.POST,
      'POST',
      {
    ...
    const afterCount = await prisma.studioPanel.count({
      where: { storyboardId: seeded.storyboard.id },
    })
