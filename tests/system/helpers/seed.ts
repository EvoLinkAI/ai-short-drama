...
export async function seedMinimalDomainState() {
  ...
  const voiceLine = await prisma.studioVoiceLine.create({
    data: {
      episodeId: episode.id,
      lineIndex: 1,
      speaker: 'Narrator',
  ...
  const secondaryPanel = await prisma.studioPanel.create({
    data: {
      storyboardId: storyboard.id,
      panelIndex: 1,
      panelNumber: 2,
  ...
  const foreignStoryboard = await prisma.studioStoryboard.create({
    data: {
      episodeId: episode.id,
      clipId: (await prisma.studioClip.create({
        data: {
          episodeId: episode.id,
  ...
  const foreignPanel = await prisma.studioPanel.create({
    data: {
